...
{
    [Header("Colliders object of interest")]
    ...
    void Update()
    {
        ...
        if (Physics.SphereCast(rayOrigin, gazeRadius, direction, out hit))
        {
            ...
            /* if(hit.collider.gameObject.transform.IsChildOf(itemLeft.transform) ||
               hit.collider.gameObject.transform.IsChildOf(itemRight.transform))
            {
...
/*             // get the hitted object name
            if (hit.collider.gameObject.tag.Equals("Foods"))
            {
                if (!hit.collider.gameObject.name.Equals("Borrelnoten"))
                {
                    hitObjectName = hit.collider.transform.parent.name;
                }
                else
                {
                    hitObjectName = hit.collider.gameObject.name;
    ...
    void LogGazeData(VarjoEyeTracking.GazeData data, VarjoEyeTracking.EyeMeasurements eyeMeasurements)
    {
        ...
        logData[32] = linkMainItem.participantID.ToString();

        logData[33] = linkMainItem.groupNumber.ToString();

        logData[34] = linkMainItem.eventType1.ToString();

        logData[35] = linkMainItem.eventNum.ToString();

        logData[36] = linkMainItem.itemLeft.ToString();

        logData[37] = linkMainItem.itemRight.ToString();

        logData[38] = linkMainItem.itemRight.ToString();

        logData[39] = linkMainItem.sideItemChosen.ToString();

        logData[40] = linkMainItem.sideItemChosen1.ToString();

        logData[41] = linkMainItem.sideItemChosen2.ToString();

        logData[42] = linkMainItem.finalItemChosen.ToString();
